            ...
            elseif idx == "FontFace" then
                ...
                if type(val) == "table" then
                    ...
                    for i,v in ipairs(val) do
                        table.insert(seq, ColorSequenceKeypoint.new(v.t or (i-1)/(#val-1), Color3.fromHex(v.h or v)))
                    end
                    self.Library.Scheme[idx] = ColorSequence.new(seq)
                    if self.Library.Options[idx] then self.Library.Options[idx]:SetValue(self.Library.Scheme[idx]) end
                else
