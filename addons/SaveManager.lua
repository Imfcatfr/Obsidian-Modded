        ...
        GradientPicker = {
            Save = function(idx, object)
                ...
                if typeof(object.Value) == "ColorSequence" then
                    for _, kp in ipairs(object.Value.Keypoints) do table.insert(seq, { t = kp.Time, h = kp.Value:ToHex() }) end
                elseif typeof(object.Value) == "table" then
                    for _, v in ipairs(object.Value) do table.insert(seq, { t = v.t or v.Time or 0, h = v.h or v.Hex or v }) end
                end
                return { type = "GradientPicker", idx = idx, val = seq }
            end,
            Load = function(idx, data)
                if SaveManager.Library.Options[idx] and type(data.val) == "table" then
                    local ok = true
                    for _, v in ipairs(data.val) do if type(v) ~= "table" or type(v.t) ~= "number" or type(v.h) ~= "string" then ok = false break end end
                    if ok then SaveManager.Library.Options[idx]:SetValue(data.val) end
                end
            end,
        },
        KeyPicker = {
