...
local Library = {
    ...
    TweenInfo = TweenInfo.new(0.35, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
    ...
    EventBus = {
        _cbs = {},
        Fire = function(self, id, ...)
            local cb = self._cbs[id]
            if cb then task.spawn(cb, ...) end
        end,
        Register = function(self, id, cb)
            self._cbs[id] = cb
        end
    },
    BlacklistedKeys = { Enum.KeyCode.W, Enum.KeyCode.A, Enum.KeyCode.S, Enum.KeyCode.D, Enum.KeyCode.Space },
    GhostKeybind = Enum.KeyCode.RightAlt,
}
...
function Library:Ripple(ui: GuiObject, x: number, y: number)
    ...
    r.Position = UDim2.fromOffset(x - abs.X, y - abs.Y)
    r.Size = UDim2.fromOffset(0,0)
    r.AnchorPoint = Vector2.new(0.5, 0.5)
    r.Parent = ui
    local mx = math.max(sz.X, sz.Y) * 2.5
    local tw = TweenService:Create(r, TweenInfo.new(0.5, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
        Size = UDim2.fromOffset(mx, mx),
        BackgroundTransparency = 1
    })
    tw:Play()
    tw.Completed:Connect(function() r:Destroy() end)
end
function Library:ApplyHoverScale(UI: GuiObject, Trigger: GuiObject)
    local scale = UI:FindFirstChildWhichIsA("UIScale")
    if not scale then scale = Instance.new("UIScale", UI) end
    local stroke = UI:FindFirstChildWhichIsA("UIStroke")
    
    Trigger.MouseEnter:Connect(function()
        if not Library.Toggled then return end
        TweenService:Create(scale, TweenInfo.new(0.3, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {Scale = 0.98}):Play()
        if stroke then TweenService:Create(stroke, TweenInfo.new(0.3, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {Thickness = 2}):Play() end
    end)
    Trigger.MouseLeave:Connect(function()
        TweenService:Create(scale, TweenInfo.new(0.4, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {Scale = 1}):Play()
        if stroke then TweenService:Create(stroke, TweenInfo.new(0.4, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {Thickness = 1}):Play() end
    end)
end

function Library:ApplyRadialGlow(UI: GuiObject, Trigger: GuiObject)
    ...
    Trigger.MouseEnter:Connect(function()
        ...
        conn = RunService.RenderStepped:Connect(function()
            ...
            local abs = UI.AbsolutePosition
            glow.Position = UDim2.fromOffset(ms.X - abs.X, ms.Y - abs.Y)
        end)
    end)
    Trigger.MouseLeave:Connect(function()
    ...
    table.insert(Library.UnloadSignals, function() if conn then conn:Disconnect() end end)
end

function Library:ApplyTextBloom(Label: TextLabel, Active: boolean)
    local bloom = Label:FindFirstChild("TextBloom")
    if Active then
        if not bloom then
            bloom = Label:Clone()
            bloom.Name = "TextBloom"
            bloom.TextTransparency = 0.8
            bloom.ZIndex = Label.ZIndex - 1
            bloom.Position = UDim2.fromOffset(0,0)
            bloom.Size = UDim2.fromScale(1,1)
            bloom:ClearAllChildren()
            bloom.Parent = Label
        end
        Label.FontFace = Font.new(Label.FontFace.Family, Enum.FontWeight.Heavy, Label.FontFace.Style)
...
function Library:UpdateColorsUsingRegistry()
    for inst, props in Library.Registry do
        for prop, idx in props do
            local sc = GetSchemeValue(idx)
            local v = sc or (typeof(idx) == "function" and idx() or idx)
            if v then
                if typeof(v) == "ColorSequence" then
                    local g = inst:FindFirstChildWhichIsA("UIGradient")
                    if not g then g = Instance.new("UIGradient", inst) end
                    g.Color = v
                    inst[prop] = Color3.new(1,1,1) -- Base color white for gradient logic
                else
                    inst[prop] = v
                    local g = inst:FindFirstChildWhichIsA("UIGradient")
...
function Library:Unload()
    if Library.ParallaxID then RunService:UnbindFromRenderStep(Library.ParallaxID) end
    table.clear(Library.EventBus)
    ...
    if Library.BlurEff then
        Library.BlurEff:Destroy()
    end
...
do
            ...
            local GetInput = nil; GetInput = function()
                ...
                if KeyName then
                    if IsMod then
                        ...
                        if Library.BlacklistedKeys and table.find(Library.BlacklistedKeys, Input.KeyCode) then
                            return GetInput()
                        end
                    end
                end
    ...
    function Funcs:AddGradientPicker(Idx, Info)
        ...
        local GradientPicker = {
            Title = Info.Title,
            Value = typeof(Info.Default) == "ColorSequence" and Info.Default or ColorSequence.new(Color3.new(1,1,1)),
            Type = "GradientPicker",
            Callback = Info.Callback,
            Changed = Info.Changed
        }
        
        function GradientPicker:SetValue(seq)
            if type(seq) == "table" and #seq > 0 then
                local arr = {}
                for i, v in ipairs(seq) do table.insert(arr, ColorSequenceKeypoint.new(v.t or v.Time or 0, Color3.fromHex(v.h or v.Hex or v))) end
                self.Value = ColorSequence.new(arr)
            elseif typeof(seq) == "ColorSequence" then
                self.Value = seq
            end
            if Idx then Library.EventBus:Fire(Idx, self.Value) end
            Library:SafeCallback(self.Changed, self.Value)
            Library:SafeCallback(self.Callback, self.Value)
        end
        
        if Idx and Info.Callback then Library.EventBus:Register(Idx, Info.Callback) end
        
        GradientPicker:SetValue(Info.Default)
...
do
    ...
    function Funcs:AddButton(...)
        ...
        local function InitEvents(Button)
            Library:ApplyHoverScale(Button.Base, Button.Base)
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        function Toggle:Display()
            ...
            Library:ApplyTextBloom(Label, Toggle.Value)
        ...
        Library:ApplyHoverScale(Button, Button)
        Library:ApplyRadialGlow(Button, Button)
    ...
    function Funcs:AddInput(Idx, Info)
        ...
        function Input:SetValue(Text)
            if Info.RegexFilter then
                Text = string.gsub(Text, Info.RegexFilter, "")
            end
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        Library.TabIndicator = New("Frame", {
            BackgroundColor3 = "AccentColor",
        ...
        New("UICorner", { CornerRadius = UDim.new(1,0), Parent = Library.TabIndicator })
    ...
    function Window:AddTab(...)
        ...
        function Tab:Show()
            ...
            if Library.TabIndicator and TabButton then
                Library.TabIndicator.BackgroundTransparency = 0
                local tarPos = UDim2.fromOffset(0, 49 + (TabButton.AbsolutePosition.Y - Tabs.AbsolutePosition.Y) + Tabs.CanvasPosition.Y)
                local stretchS = UDim2.fromOffset(4, TabButton.AbsoluteSize.Y * 1.5)
                local normS = UDim2.fromOffset(4, TabButton.AbsoluteSize.Y)
                
                TweenService:Create(Library.TabIndicator, TweenInfo.new(0.2, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
                    Position = tarPos,
                    Size = stretchS
                }):Play()
            ...
            Library:ApplyTextBloom(TabLabel, true)
        ...
        function Tab:Hide()
            Library:ApplyTextBloom(TabLabel, false)
            TweenService:Create(TabButton, Library.TweenInfo, {
                BackgroundTransparency = 1,
            }):Play()
            TweenService:Create(TabLabel, Library.TweenInfo, {
    ...
    local function updScl()
        ...
        local scl = math.clamp(math.min(vp.X / 1920, vp.Y / 1080), 0.5, 2)
        for _, s in pairs(Library.Scales) do s.Scale = Library.DPIScale * scl end
    end
    Library:GiveSignal(workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"):Connect(updScl))
    updScl()

    Library.BasePosition = WindowInfo.Position
    Library.ParallaxID = "ObsidianParallax"
    RunService:BindToRenderStep(Library.ParallaxID, 1, function()
        if not (ScreenGui and ScreenGui.Parent) or Library.ActiveLoading then return end
        local ms = UserInputService:GetMouseLocation()
        local vp = workspace.CurrentCamera.ViewportSize
        local x = (ms.X / vp.X - 0.5) * 6
        local y = (ms.Y / vp.Y - 0.5) * 6
        if MainFrame and MainFrame.Visible and not Library.CantDragForced then
            MainFrame.Position = Library.BasePosition + UDim2.fromOffset(x, y)
        end
    end)

    Library:GiveSignal(UserInputService.InputBegan:Connect(function(inp)
        if inp.KeyCode == Library.GhostKeybind then
            if ScreenGui.Parent then
                ScreenGui.Parent = nil
                if Library.BlurEff then Library.BlurEff.Size = 0 end
            else
    ...
    local function bootSeq()
        ...
        grad.Rotation = 45
        grad.Parent = MainFrame:FindFirstChildWhichIsA("UIStroke") or MainFrame
        
        local blur = Instance.new("BlurEffect", game:GetService("Lighting"))
        blur.Size = 0
        Library.BlurEff = blur
        TweenService:Create(blur, TweenInfo.new(1), {Size = 15}):Play()
        
        local tw = TweenService:Create(grad, TweenInfo.new(1.5, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {Offset = Vector2.new(1, 0)})
        tw:Play()
        tw.Completed:Wait()
        grad:Destroy()
